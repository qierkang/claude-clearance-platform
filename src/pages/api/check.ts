...
function siteFromUrl(url: URL): string {
  return url.origin;
}

function jsonBody(a: Analysis, lang: Lang, site: string) {
  const t = useTranslations(lang);
  return {
    app: 'Claude Clearance',
    estimate: true,
    lang,
    score: a.score,
    band: a.band,
...
function textBody(a: Analysis, lang: Lang, color: boolean, site: string): string {
  const t = useTranslations(lang);
  ...
  const out: string[] = [];
  out.push(rule('╭'));
  out.push(`${bar}  ${accent(bold('Claude Clearance'))}  ${dim(L.subtitle)}`);
...
export const GET: APIRoute = ({ request, url }) => {
  ...
  if (!wantsJson(url, request)) {
    return new Response(textBody(analysis, lang, wantsColor(url, request), site), {
      status: 200,
      headers: {
  ...
  return new Response(JSON.stringify(jsonBody(analysis, lang, site), null, 2), {
    status: 200,
    headers: {
